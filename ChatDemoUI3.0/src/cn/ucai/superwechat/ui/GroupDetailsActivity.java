...
public class GroupDetailsActivity extends BaseActivity implements OnClickListener {
    ...
    private void findAppGroup() {
        NetDao.findGroupByHxid(this, group.getGroupId(), new OkHttpUtils.OnCompleteListener<String>() {
            @Override
            public void onSuccess(String s) {
                if (s != null) {
                    Result result = ResultUtils.getResultFromJson(s, Group.class);
                    if (result != null && result.isRetMsg()) {
                        appGroup = (Group) result.getRetData();
                        L.e(TAG,"findAppGroup="+appGroup.getMGroupId());
                        if (appGroup==null){
    ...
    private void updateAppGroupName(String returnData) {
        NetDao.updateGroupName(this, appGroup.getMGroupId().toString(), returnData, new OkHttpUtils.OnCompleteListener<String>() {
            @Override
            public void onSuccess(String s) {
                if (s != null) {
                    Result result = ResultUtils.getResultFromJson(s, Group.class);
                    if (result != null && result.isRetMsg()) {
                        appGroup = (Group) result.getRetData();
                        L.e(TAG,"updateAppGroupName"+appGroup.getMGroupId());
                    } else {
                        Toast.makeText(GroupDetailsActivity.this, "更新名称失败", Toast.LENGTH_SHORT).show();
                    }
                } else {
                    Toast.makeText(GroupDetailsActivity.this, "更新名称失败", Toast.LENGTH_SHORT).show();
                }
                ...
                try {
                    ...
                    runOnUiThread(new Runnable() {
                        public void run() {
                            progressDialog.dismiss();
                            setResult(RESULT_OK);
                            deleteAppGroupMember(EMClient.getInstance().getCurrentUser());
                            finish();
    ...
    private void deleteAppGroup() {
        NetDao.deleteGroup(this, appGroup.getMGroupId().toString(), new OkHttpUtils.OnCompleteListener<String>() {
            @Override
            public void onSuccess(String s) {
                if (s != null) {
                    Result result = ResultUtils.getResultFromJson(s, Result.class);
                    L.e(TAG,"deleteAppGroup="+appGroup.getMGroupId());
                    if (result != null && result.isRetMsg()) {
                        Toast.makeText(GroupDetailsActivity.this, "解散群组成功", Toast.LENGTH_SHORT).show();
                    }else {
                        Toast.makeText(GroupDetailsActivity.this, "解散群组失败", Toast.LENGTH_SHORT).show();
                    }
                }else {
                    Toast.makeText(GroupDetailsActivity.this, "解散群组失败", Toast.LENGTH_SHORT).show();
                }
            ...
            NetDao.addGroupMembers(this, group, new OkHttpUtils.OnCompleteListener<String>() {
                ...
                public void onSuccess(String s) {
                    if (s != null) {
                        ...
                        if (result != null && result.isRetMsg()) {
                            appGroup = (Group) result.getRetData();
                            Toast.makeText(GroupDetailsActivity.this, "添加好友成功", Toast.LENGTH_SHORT).show();
                        } else {
                            Toast.makeText(GroupDetailsActivity.this, "添加好友失败", Toast.LENGTH_SHORT).show();
                        }
                    } else {
                        Toast.makeText(GroupDetailsActivity.this, "添加好友失败", Toast.LENGTH_SHORT).show();
                    }
...
//				button.setCompoundDrawables(null, avatar, null, null);
    ...
    private void deleteAppGroupMember(String username) {
        NetDao.deleteGroupMember(this, appGroup.getMGroupId().toString(), username, new OkHttpUtils.OnCompleteListener<String>() {
            @Override
            public void onSuccess(String s) {
                if (s != null) {
                    Result result = ResultUtils.getResultFromJson(s, Group.class);
                    if (result != null && result.isRetMsg()) {
                        appGroup = (Group) result.getRetData();
                        L.e(TAG,"deleteAppGroupMember="+appGroup.getMGroupId());
